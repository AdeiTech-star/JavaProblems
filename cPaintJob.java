...
public class cPaintJob {
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets){
        ...
        if(width<=0 || height <=0 || areaPerBucket <=0 || extraBuckets<0){
            return -1;
             ...
             bucketPerArea = wallArea / areaPerBucket;
             numOfBuckets = (int) Math.ceil(bucketPerArea - extraBuckets);
        }
        return numOfBuckets;
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket){
        ...
        if(width<=0 || height <=0 || areaPerBucket <=0){
            ...
            wallArea = width * height;
            bucketPerArea = wallArea/areaPerBucket;
            numOfBuckets = (int)(Math.ceil(bucketPerArea));
    ...
    public static int getBucketCount (double area, double areaPerBucket){
        ...
        if(area<=0 || areaPerBucket <=0){
            ...
            numOfBudgets = (int)(Math.ceil((area / areaPerBucket)));
        }
        return numOfBudgets;
